...
# def get_relevant_papers(query_title, query_abstract, key):
...
#     id_list = np.argsort(-np.dot(emb, emb_matrix.T), axis=1)[0][:15]
#     # create a dict with titles, authors, year, and link:
#     return id_list

# def generate_literature(query_title, query_abstract, id_list, key):
#     embeddings = OpenAIEmbeddings(model="text-embedding-ada-002", openai_api_key=key)
#     citations = []
